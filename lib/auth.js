...
export const auth = betterAuth({
  ...
  // Grant the sign-up bonus exactly once, right after the user record is
  // created (covers both email/password AND Google sign-in/sign-up).
  databaseHooks: {
    user: {
      create: {
        after: async (user) => {
          if (user.creditsGranted) return;
          const bonus = user.role === "creator" ? 20 : 50;
          await db.collection("user").updateOne(
            { _id: user._id ?? user.id },
            { $set: { credits: bonus, creditsGranted: true } }
          );
        },
      },
    },
